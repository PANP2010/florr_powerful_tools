...
| florr-auto-framework-pytorch | AI模型训练框架 | PyTorch, YOLO26, Ultralytics | 高 |
| florr-auto-sszone | 自动刷怪脚本 | Python, YOLOv10, PaddlePaddle | 高 |
| upload_package | 云端训练包 | YOLO26s, Kaggle/Colab | 高 |
| train_package | 本地训练包 | YOLO, PyTorch | 高 |
| florr_knowledge_base | 游戏知识库 | Python, Wiki爬虫 | 中 |
| florr_assistant | 整合工具 | PyQt5, 多模块集成 | 高 |
...
└───────────────┘    └───────────────┘    └───────────────┘
        ...
        ▼                       ▼                       ▼
┌───────────────┐    ┌───────────────┐    ┌───────────────┐
│  YOLO Models │    │  PyTorch NN   │    │ YOLO26s      │
│  (目标检测)   │    │  (神经网络)    │    │ (最新模型)    │
└───────────────┘    └───────────────┘    └───────────────┘
                                │
                                ▼
...
| 目标检测 | YOLO/Ultralytics | 8.4+ (YOLO26) | 实时目标检测 |
| Web框架 | FastAPI | 0.x | WebSocket服务器 |
| GUI框架 | Tkinter/PyQt5 | 内置/5.15+ | 图形界面 |
...
│   └── default.yaml          # 默认配置
├── docs/                      # 文档目录
│   ├── FREE_GPU_TRAINING_GUIDE.md  # 免费GPU训练指南
│   ├── QUICK_START_TRAINING.md     # 快速开始训练
│   ├── UI_INTEGRATION_GUIDE.md      # UI集成指南
│   └── OVERLAY_README.md            # 悬浮窗说明
...
| 日期 | 模块 | 更改内容 | 状态 |
|------|------|---------|------|
| 2026-03-14 | 训练系统 | 升级到YOLO26s模型，创建云端训练脚本 | ✅ 完成 |
| 2026-03-14 | 训练系统 | 创建train_package和upload_package训练包 | ✅ 完成 |
| 2026-03-14 | 知识库 | 创建florr_knowledge_base自动更新知识库 | ✅ 完成 |
| 2026-03-14 | 文档 | 更新FREE_GPU_TRAINING_GUIDE和MODEL_DOWNLOAD文档 | ✅ 完成 |
...
## YOLO26s 训练系统 (2026-03 新增)

### 技术升级

项目已升级到 Ultralytics YOLO26s 最新模型（2025年发布）：
...
```
upload_package/              # 云端训练包（推荐）
├── mob_images/              # 77种Mob图像
├── backgrounds/             # 地图背景图像
├── dataset/                 # 验证集
│   ├── val/labels/          # 300+验证标签
│   ├── data.yaml            # 数据集配置
│   └── data_cloud.yaml      # 云端配置
├── scripts/
...
| 平台 | GPU型号 | 显存 | 时长限制 | 推荐配置 |
|------|---------|------|----------|----------|
| Kaggle | P100/T4 | 13-15GB | 30小时/周 | 5000样本, batch=16 |
| Google Colab | T4/P100 | 15GB | 12小时/次 | 5000样本, batch=16 |
| 恒源云 | 3090/4090 | 24GB+ | 按量付费 | 10000样本, batch=32 |
...
## Florr Knowledge Base (2026-03 新增)

### 功能概述

自动更新的 Florr.io 游戏知识库组件，支持从 Wiki 自动获取游戏数据。
...
| 模块 | 完成度 | 状态 | 最新更新 |
|------|--------|------|----------|
| florr-auto-afk | 88% | ✅ 生产可用 | macOS兼容性优化 |
| florr-auto-framework-pytorch | 90% | ✅ 生产可用 | YOLO26s升级 |
| florr-auto-pathing | 78% | ⚠️ 开发中 | 地图分类器完成 |
| florr-auto-sszone | 82% | ✅ 生产可用 | 配置完善 |
| florr_assistant | 75% | ⚠️ 开发中 | UI框架完成 |
| upload_package | 95% | ✅ 可用 | 云端训练就绪 |
| train_package | 95% | ✅ 可用 | 本地训练就绪 |
| florr_knowledge_base | 90% | ✅ 可用 | 知识库完成 |
...
|------|------|------|
| 主架构文档 | PROJECT_ARCHITECTURE.md | 本文档 |
| 云端训练脚本 | upload_package/scripts/train_cloud.py | YOLO26s训练 |
| 本地训练脚本 | train_package/scripts/train.py | 本地训练 |
| 知识库模块 | florr_knowledge_base/ | 游戏数据 |
| AFK主程序 | florr-auto-afk-main/segment.py | AFK检测入口 |
| 训练框架 | florr-auto-framework-pytorch/ | AI训练 |
| 整合工具 | florr_assistant/ | 统一入口 |
